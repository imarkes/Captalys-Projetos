...
domain = '{{var.value.domain}}'
technology = 'sftp'
hostname = '{{var.value.hostname}}'
username = '{{var.value.username}}'
password = '{{var.value.password}}'
path_doman = '{{var.value.path_doman}}'
...
with DAG(
    ...
    for prefix in lista_prefix:
        tasks_glueJob_download_sftp.append(
            AwsGlueJobOperator(
                task_id=f"glueJob_download_sftp_{prefix[0]}",
                pool='datalake_glueJob',
        ...
        tasks_glueJob_unzip_file.append(
            AwsGlueJobOperator(
                task_id=f"glueJob_unzip_file_{prefix[0]}",
                pool='datalake_glueJob',
                job_name='datalake-sftp',
                script_location='s3://captalys-analyticsscripts-production/',
                script_args={
