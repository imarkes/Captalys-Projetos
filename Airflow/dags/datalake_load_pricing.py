...
def notification(context):
    ...
    data = {
        'subject': 'Error on DAG datalake_load_pricing in client Captalys',
        'error_msg': 'Error on DAG datalake_pricing',
...
with DAG(
        dag_id='datalake_load_pricing',
        schedule_interval='0 8 * * *',
        start_date=datetime(2022, 5, 16),
        default_args=default_args,
        catchup=False,
        on_failure_callback=notification,
        tags=['roxpartner', 'datalake', 'recebiveis', 'pricing']
) as dag:
    ...
    task_glueCrawler_raw_pricing = AwsGlueCrawlerOperator(
        task_id='glueCrawler_raw_pricing',
        config={
            'Name': 'datalake-raw-pricing'
        },
        aws_conn_id='datalake-aws-analytics'
    )

    task_redshift_insert_glueCrawler_raw_pricing = RedshiftSQLOperator(
        task_id='redshift_insert_glueCrawler_raw_pricing',
        sql="""
            INSERT INTO log.airflow_log (job_name, inicio, fim) VALUES ( 
                'glueCrawler_raw_pricing', 
                '{{ dag_run.get_task_instance('glueCrawler_raw_pricing').start_date }}', 
                '{{ dag_run.get_task_instance('glueCrawler_raw_pricing').end_date }}'
            );
        """,
        redshift_conn_id=redshift_conn_id
    )

    task_glueCrawler_raw_pricing_err = AwsGlueCrawlerOperator(
        task_id='glueCrawler_raw_pricing_err',
        config={
            'Name': 'datalake-raw-pricing-err'
        },
        aws_conn_id='datalake-aws-analytics'
    )

    task_redshift_insert_glueCrawler_raw_pricing_err = RedshiftSQLOperator(
        task_id='redshift_insert_glueCrawler_raw_pricing_err',
        sql="""
            INSERT INTO log.airflow_log (job_name, inicio, fim) VALUES ( 
                'glueCrawler_raw_pricing_err', 
                '{{ dag_run.get_task_instance('glueCrawler_raw_pricing_err').start_date }}', 
                '{{ dag_run.get_task_instance('glueCrawler_raw_pricing_err').end_date }}'
            );
        """,
        redshift_conn_id=redshift_conn_id
    )
    ...
    (
            ...
            >> task_glueCrawler_raw_pricing
            >> [tasks_redshift_insert_glueJob_trusted[i] << tasks_glueJob_trusted[i] for i in
                range(len(tasks_glueJob_trusted))]
    )

    task_glueCrawler_raw_pricing >> task_glueCrawler_raw_pricing_err
    task_glueCrawler_raw_pricing >> task_redshift_insert_glueCrawler_raw_pricing
    task_glueCrawler_raw_pricing_err >> task_redshift_insert_glueCrawler_raw_pricing_err
